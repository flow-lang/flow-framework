...
const UPDATE_CUTOFF = 'update-cutoff'
...
function update({ action, payload }, model) {
  switch (action) {
    ...
    case UPDATE_CUTOFF: {
      const { freq } = payload
      const synth = { ...model.synth, cutoff: freq }
...
function listen(model) {
  const listeners = [
    ...
    DOM.Event.input('[data-cutoff]', throttle(100, ({ target }) => {
      return Action(UPDATE_CUTOFF, { freq: Number(target.value) })
    })),
